...
- (void)viewDidLoad {
    ...
    [self.myButton setTitle:@"方法注入" forState:UIControlStateNormal];
    
    [self.view addSubview:self.myButton];
    
    [self.myButton addTarget:self action:@selector(right_button_event:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)right_button_event:(UIButton *)sender
{
    ...
    //利用关联 封装BLOCK调用
    UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"提示" message:@"给弹框添加了 actionBlock, 看打印" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
...
-(NSString *)getClassInfo
{
    ...
    [strM appendString:[NSString stringWithUTF8String:class_getName([self.myRunTimeTest class])]];
    [strM appendString:[NSString stringWithUTF8String:class_getName(class_getSuperclass([self.myRunTimeTest class]))]];
    [strM appendString:(class_isMetaClass([self.myRunTimeTest class]) ? @"" : @"not")];
    [strM appendString:[NSString stringWithUTF8String:class_getName([LMJRunTimeTest class])]];
    [strM appendString:[NSString stringWithUTF8String:class_getName(meta_class)]];
    
    [strM appendFormat:@"%zu", class_getInstanceSize([self.myRunTimeTest class])];
...
- (UITextView *)inputTextView
{
    if(_inputTextView == nil)
    {
        ...
        [textView makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.edges.equalTo(UIEdgeInsetsMake(64, 0, 60, 0));
