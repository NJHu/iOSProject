...
@interface LMJUpLoadProgressViewController ()<LMJElementsFlowLayoutDelegate, UIActionSheetDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, QBImagePickerControllerDelegate, MWPhotoBrowserDelegate>
...
- (void)rightButtonEvent:(UIButton *)sender navigationBar:(LMJNavigationBar *)navigationBar
{
    ...
    [self.uploadImageHelper.imagesArray enumerateObjectsUsingBlock:^(MPImageItemModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        
        LMJUploadImagesService *uploadService = [LMJUploadImagesService new];
        uploadService.imagesArray = [NSMutableArray arrayWithObject:obj.image];
        
        if (obj.uploadState == MPImageUploadStateSuccess || obj.uploadState == MPImageUploadStateIng) {
            return ;
        }
        ...
        [uploadService uploadWithProgress:^(NSProgress *progress) {
            ...
            obj.uploadProgress = (double)progress.completedUnitCount / progress.totalUnitCount;
            
            obj.uploadState = MPImageUploadStateIng;
            
            NSLog(@"%@", [NSString stringWithFormat:@"%.2f", (double)progress.completedUnitCount / progress.totalUnitCount]);
            
        } completion:^(LMJBaseResponse *response) {
            
            if (response.error) {
                
                obj.uploadState = MPImageUploadStateFail;
                
            }else
            {
                obj.uploadState = MPImageUploadStateSuccess;
                [obj setStringProperty:@"1"];
                // 获取路径
                obj.upServicePath = [response.responseObject description];
            }
